...
def redact_text(data: TextInput):
    ...
    patterns = {
        # Email addresses
        r"\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b": "[redact]",
       
        # South African ID numbers (13 digits, often start with YYMMDD)
        r"\b\d{2}(0[1-9]|1[0-2])(0[1-9]|[13]\d|3[01])\d{7}\b": "[redact]",
        # Phone numbers (+27, 0xx, etc.)
        r"(\+?\d{1,3}[-.\s]?)?(\(?\d{2,3}\)?[-.\s]?)?\d{3,4}[-.\s]?\d{4}": "[redact]",
        # Names (very simple heuristic: "Firstname Lastname")
        r"\b[A-Z][a-z]+ [A-Z][a-z]+\b": "[redact]",
         # Credit card numbers (Visa, MasterCard, etc.)
        r"\b(?:\d[ -]*?){7,9}\b": "[redact]",
        # Street addresses (simple heuristic)
        r"\d{1,5} [A-Za-z ]+(Street|St|Avenue|Ave|Road|Rd|Lane|Ln|Drive|Dr)\b": "[redact]",
    }
